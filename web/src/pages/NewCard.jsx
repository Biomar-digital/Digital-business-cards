...
export default function NewCard() {
  ...
  return (
    <>
      <div className="page-head"><h1>New card</h1></div>
      <form className="card" style={{ maxWidth: 640 }} onSubmit={submit}>
        ...
        <div className="row">
          <div className="field"><label>Job title</label><input value={form.jobTitle} onChange={set('jobTitle')} /></div>
        ...
        <div className="field"><label>Website</label><input value={form.website} onChange={set('website')} /></div>
        <div className="field">
          <label>Group</label>
        ...
        <div className="field"><label>Notes</label><textarea rows={3} value={form.notes} onChange={set('notes')} /></div>
        <div className="field checkbox">
          <input id="se" type="checkbox" checked={form.sendEmail}
            onChange={(e) => setForm((f) => ({ ...f, sendEmail: e.target.checked }))} />
          <label htmlFor="se" style={{ margin: 0 }}>Email the pass to the contact on creation</label>
        </div>

        {error && <p style={{ color: 'var(--red)' }}>{error}</p>}
        <button className="btn" disabled={saving}>
          {saving ? 'Creating…' : 'Create pass + QR'}
        </button>
      </form>
    </>
  )
